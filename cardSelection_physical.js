...
function draw()
{
    if(PS_playerCount <= PS_totalPlayers)
    {
        ...
        else if(PS_instOverlayFlag == 0.5)
        {
            ...
            //generating game logo on placeholder rectangles
            let PS_gameLogo_size = width/9;
            for(let i=1; i<PS_totalCards; i++)
            {
                imageMode(CENTER);
                image(PS_gameLogo_BlackRed, PS_cards_initxPos, height/1.8, PS_gameLogo_size, PS_gameLogo_size/4);
                PS_cards_initxPos += PS_cards_separation;
            }
...
function instructionContent(overlayFlagValue)
{
    ...
    switch(overlayFlagValue)
    {
        case 1:
            ...
            imageMode(CORNER);
            image(PS_instructionsImage, PS_instImg_startPos, height/5.25, PS_instImg_size, PS_instImg_size/1.79);
        break;
        case 2:
