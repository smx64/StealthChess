...
class GP_Chessboard
{
    //constructor function for initializing all chessboard-related variables & flags
    ...
    GP_drawBlock()
    {
        ...
        //conditional statements to alternate b/w red-white fill colored blocks
        if(this.GP_block_idRow%2 == 0)
        {
            if(this.GP_block_idCol%2 != 0)
            {
                this.GP_block_fillColor = PS_redShade;
            }
            else
            {
        ...
        {
            this.GP_block_fillColor = color(190,255,255);
        }
...
function gamePlay()
{
    //starting background score for the gameplay segment
    GP_bgm.setVolume(0.25);
...
function GP_bishopChecks(thisRow, thisColumn)
{
    ...
    for(let i=thisRow-1, j=thisColumn-1; (i>=0 && i<PP_chessboardSize && j>=0 && j<PP_chessboardSize); i--, j--)
    {
        ...
        else if(GP_blocksArray[i][j].GP_block_occupiedFlag == 1)
        {
            ...
            if(GP_blocksArray[i][j].GP_block_pieceType == 'K' && GP_blocksArray[i][j].GP_block_playerNumber == GP_playerActive)
            {
                GP_blocksArray[i][j].GP_block_checkFlag = 1;
            }
            else
...
function GP_rookChecks(thisRow, thisColumn)
{
    ...
    for(let i=thisColumn+1; i<PP_chessboardSize; i++)
    {
        ...
        {
            //code to not stop check-block checking if opponent's king is encountered
            if(GP_blocksArray[thisRow][i].GP_block_pieceType == 'K' && GP_blocksArray[thisRow][i].GP_block_playerNumber == GP_playerActive)
            {
                GP_blocksArray[thisRow][i].GP_block_checkFlag = 1;
            }
            else
    ...
    for(let i=thisRow+1; i<PP_chessboardSize; i++)
    {
        ...
        {
            //code to not stop check-block checking if opponent's king is encountered
            if(GP_blocksArray[i][thisColumn].GP_block_pieceType == 'K' && GP_blocksArray[i][thisColumn].GP_block_playerNumber == GP_playerActive)
            {
                GP_blocksArray[i][thisColumn].GP_block_checkFlag = 1;
            }
            else
...
function GP_playerTextDisplay()
{
    ...
    //enabling instructional overlay only during active gameplay
    if(GP_winFlag == 0)
    {
        ...
        rectMode(CORNER);
        fill(PS_redShade);
        rect(width/1.25, height/16, width, height/19);
        textAlign(RIGHT,CENTER);
        textFont(PS_fontBody);
        textSize(25);
        fill(255);
        text("Press [ i ] to view instructions", width/1.01, height/13.3);
    }
}
